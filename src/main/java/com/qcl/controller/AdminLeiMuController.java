...
public class AdminLeiMuController {
    ...
    //类目详情页
    @GetMapping("/detail")
    public String index(@RequestParam(value = "leimuId", required = false) Integer leimuId,HttpServletRequest req,
                        ModelMap map) {
        if (leimuId != null) {
        ...
        // 将管理员的信息添加到模型中
        boolean canEdit = admin.getAdminType() != AdminStatusEnum.YUANGONG.getCode();
     ...
     * */
    @GetMapping("/excel")
    public String excel(HttpServletRequest req,ModelMap map) {
        AdminInfo admin = (AdminInfo) req.getSession().getAttribute("admin");
        if (admin == null) {
            // 如果admin对象是null，重定向用户到登录页面
            // return "redirect:/diancan/loginView";
